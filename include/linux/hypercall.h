...
static inline uint32_t igloo_hypercall2(uint32_t num, uint32_t a1, uint32_t a2) {
  ...
  return r0;
    ...
    // MIPS specific code
    register uint32_t a0 asm("a0") = num;  // Set up a0 with the value of num
    register uint32_t a1 asm("a1") = a1;   // Argument 2
    register uint32_t a2 asm("a2") = a2;   // Argument 3

    asm volatile(
       "movz $0, %[a0], %[a1]"
        : "+r"(a0)  // Input and output
        : [a1] "r" (a1), [a2] "r" (a2) // Input only
        :  // Clobber list is empty because a0 is specified as an input-output operand
    );

    // Read the result from a0
    return a0;
#else
